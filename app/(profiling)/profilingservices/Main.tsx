...
import { fetchServices } from '@/utils/fetchApi'
import React, { useEffect, useState } from 'react'

// Types
import type { ServicesListTypes } from '@/types'
...
const Page: React.FC = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<ServicesListTypes | null>(
    null
  )

  // List
  const [list, setList] = useState<ServicesListTypes[]>([])
  ...
  const fetchData = async () => {
    ...
    try {
      const result = await fetchServices(perPageCount, 0)
  ...
  const handleShowMore = async () => {
    ...
    try {
      const result = await fetchServices(perPageCount, list.length)
  ...
  const handleEdit = (item: ServicesListTypes) => {
    setShowAddModal(true)
